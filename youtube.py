...
def generate_video_sitemap():
    ...
    f = open(os.path.join(request.folder,'static/video_sitemap.xml'), 'w')
        ...
        <url>
         ...
         <video:video>
           ...
           <video:title>%s</video:title>
            ...
            ''' % ('http://www.gogocow.com/downloader/youtube/detail?url=', urllib2.quote(url.movie_url), 'http://www.gogocow.com/downloader/youtube/upload?url=%s%%26image=%s' % (urllib2.quote(url.movie_url), url.movie_image) , url.movie_title.replace('&','%26').replace('&','%26').replace('<','').replace('>','') )
...
def index():
    ...
    if key == 'youtube':
        ...
        hot_rows = db(db.keywords_publish.id).select(orderby=~db.keywords_publish.request_time, limitby=(0,7))
        c['hot_rows'] = hot_rows
#	seo = db(db.recently_download.id).select(groupby=db.recently_download.movie_title)
#	return "123"
#	c['seo'] = seo
